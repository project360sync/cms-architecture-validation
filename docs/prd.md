...
| 1 — Greenfield v1 spike (átstrukturálás) | A §15.3/(A) fixed-only mag a §16 specek szerint + a GSAP × reflow kockázat mérése valós referenciaoldalon — a bulletproof mag az előfeltétele a meglévő kliensek bevonásának [D6] | Indulhat azonnal [D6]: az AI-asszisztált build sebessége megdöntötte a demand-first költség-premisszát; a Gate A párhuzamosan fut, és a TELJES platform-elköteleződést (Phase 3) továbbra is blokkolja | Gate B teljesül: determinisztikus mount/refresh/destroy + hosszváltozás a rögzített mátrixon, manuális újrahangolás nélkül (§9 Gate B küszöb) |
...
Gate A — Demand. (D6 után: párhuzamos sáv — a spike-ot nem blokkolja, a Phase 3 teljes platform-elköteleződést igen. Az evidencia elsődleges forrása: meglévő, már kiadott oldalak átültetése az új CMS mögé, majd valós kliens-használat mérése; a (c) helyettesítés-teszt helyett dokumentált összehasonlítás készül [D6].)
...
| D6 | Szekvenálás-revízió: blokkolja-e a Gate A a spike-ot | Nem blokkolja — párhuzamos sáv; indok: az AI-asszisztált build (5–6 havi munkaérték napok alatt) megdöntötte a demand-first költség-premisszát; a bulletproof átstrukturálás előfeltétele a meglévő kliensek bevonásának; Gate A evidencia meglévő kiadott oldalak CMS mögé ültetéséből + valós használat méréséből; trade-off: (c) helyettesítés-teszt → dokumentált összehasonlítás. A Phase 3 teljes platform-elköteleződést a Gate A továbbra is blokkolja. | confirmed | user answer, 2026-07-24 |
| D7 | Spike-implementáció helye + újrafelhasználási politika | claude-cms ágon, újraírás-elvvel: ha egy fájl nagyobb része nem használható, az egész deprecated és újraírandó; a régi struktúra nem szoríthat keretek közé; nincs visszafelé kompatibilitás (nem éles) | confirmed | user answer, 2026-07-24 |

Technical design pointer. A HOGYAN külön dokumentumban él (sibling `tech-spec-template.md`). A sorrendet a D6 döntés újrarendezte: a spike-implementáció azonnal indulhat (a §16 specek töltik be a spike-mélységű spec szerepét), a Gate A párhuzamosan fut, a teljes platform-elköteleződés (Phase 3) marad kereslet-evidenciához kötve [D6]. Implementációs részlet ne kerüljön ebbe a PRD-be; ide linkelve.
